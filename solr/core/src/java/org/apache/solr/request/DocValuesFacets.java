...
public class DocValuesFacets {
     ...
     */
    static NamedList<Integer> finalize(NamedList<Integer> res, SolrIndexSearcher searcher, SchemaField
            schemaField, DocSet docs, int missingCount, boolean missing) throws IOException {
        if (missing) {
            if (missingCount < 0) {
     ...
     */
    static byte[] accumSingleSeg(int[] counts, SortedDocValues si, DocIdSetIterator disi, int subIndex, OrdinalMap map, boolean cachePerSeg) throws
            IOException {
